...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @forelse($produkTerbaru ?? [] as $produk)
                    ...
                    <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                        ...
                        <a href="{{ route('transaksis.create', ['id_jasa' => $produk->id_jasa]) }}" 
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm">
                            <a href="{{ route('transaksis.show', $jadwal->id_transaksi) }}" class="text-blue-600 hover:text-blue-800">Detail</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-semibold text-gray-900">📋 Riwayat Transaksi Terbaru</h3>
            <a href="{{ route('transaksis.index') }}" class="text-sm text-blue-600 hover:text-blue-800">Lihat Semua →</a>
        </div>
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 pb-2">
                <div>
                    <p class="font-medium text-gray-900">{{ $trx->nomor_unik }}</p>
                    <p class="text-sm text-gray-500">{{ $trx->nama_pelanggan }} - {{ $trx->product->nama_jasa ?? '-' }}</p>
                    @if($trx->tanggal_booking)
                        <p class="text-xs text-gray-400">Booking: {{ $trx->tanggal_booking->format('d/m/Y H:i') }}</p>
                    @endif
                </div>
                <div class="text-right">
                    <p class="font-semibold text-gray-900">Rp {{ number_format((float) ($trx->total_harga ?? 0), 0, ',', '.') }}</p>
                    <div class="flex items-center space-x-2 mt-1">
                        @if($trx->status_pembayaran == 'lunas')
                            <span class="text-xs text-green-600">✓ Lunas</span>
                            <a href="{{ route('transaksis.struk', $trx->id_transaksi) }}" class="text-xs text-blue-600 hover:text-blue-800">Cetak Struk</a>
                        @else
                            <span class="text-xs text-yellow-600">⏳ Belum Bayar</span>
                            <a href="{{ route('transaksis.bayar', $trx->id_transaksi) }}" class="text-xs text-green-600 hover:text-green-800">Bayar</a>
                        @endif
                    </div>
                </div>
            </div>
