        ...
        <div class="p-8">
            ...
            <div class="flex items-center space-x-4 pt-4 border-t border-gray-200">
                @if($transaksi->status_pembayaran == 'belum' && (auth()->user()->isKasir() || auth()->user()->isAdmin()))
                    <a href="{{ route('transaksis.bayar', $transaksi->id_transaksi) }}" 
                ...
                @if($transaksi->status_pembayaran == 'lunas')
                    <a href="{{ route('transaksis.struk', $transaksi->id_transaksi) }}" 
                ...
                @if($transaksi->status == 'pending' && (auth()->user()->isKasir() || auth()->user()->isAdmin()))
                    <form action="{{ route('transaksis.batal', $transaksi->id_transaksi) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin membatalkan transaksi ini?')">
                        @csrf
                        @method('POST')
